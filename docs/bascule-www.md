...
- [x] ✅ **Deux vrais paiements validés de bout en bout — 22/09/2026**
      ...
      ✅ **EasyBeer a bien reçu la commande** (confirmé par Guillaume), et
      **les deux commandes de test sont remboursées** — #26537 via
      WooPayments, avec `_wcpay_refund_status: successful` (un vrai
...
- [x] ✅ **Champs personnalisés du checkout — répondu par les faits (22/09/2026)**

      L'extension **Checkout Field Editor for WooCommerce** est active sur le
      WP. Elle permet d'ajouter, de modifier ou de rendre obligatoires des
      champs sur la page de commande — et le checkout Astro, lui, n'envoie que
      les champs standard de la Store API.

      🔑 **La preuve ne vient pas des réglages, elle vient des commandes.**
      #26535 (PayPal) et #26537 (carte) sont toutes deux `created_via:
      store-api` — donc passées par le checkout Astro — et toutes deux
      **acceptées et payées**. Si un champ obligatoire manquait, WooCommerce
      aurait refusé la commande au lieu de l'encaisser.

      Leurs `meta_data` le confirment côté contenu : aucun champ personnalisé,
      seulement `is_vat_exempt`, les hachages de panier, les métadonnées de
      paiement et le suivi. Même constat sur quatre commandes passées par le
      checkout **WordPress** (#26042, #26012, #25953, #25926).

      ⚠️ **À re-vérifier uniquement si quelqu'un touche aux réglages de
      l'extension.** Le contrôle est alors le même : passer une commande de
      bout en bout depuis Astro. Lire les réglages n'est pas possible — le MCP
