...
/**
 * Awards — composant inspiré de 21st.dev (aliimam/award).
 *
 * Adapté pour La Brasserie des Plantes :
 *   - Le variant `badge` (seul utilisé en prod actuellement) respecte le
 *     niveau de la médaille via une palette métallique or / argent / bronze /
 *     platine. Le bouclier SVG, les couleurs de titre et de bordure
 *     s'adaptent au `level` fourni en prop.
 *   - Les autres variants (stamp / award / certificate / sticker / id-card)
 *     sont conservés intacts depuis 21st.dev pour un usage futur. Si la
 *     page ne déclenche pas ce variant, leur JSX n'est jamais monté dans
 *     le DOM — pas de coût runtime, juste un peu de bundle JS.
 */
...
/**
 * Palette métallique pour le variant `badge` — couleurs « vraies » (or, argent,
 * bronze, platine) appliquées au bouclier SVG, au titre et aux filets. Ça
 * permet à une rangée de médailles de bien se lire : un coup d'œil suffit
 * pour distinguer une médaille d'or d'une médaille d'argent.
 */
const badgePalette: Record<
  NonNullable<AwardsComponentProps["level"]>,
  { fill: string; title: string; border: string; bg: string; subtitle: string; accent: string }
> = {
  gold: {
    fill: "#c9a34a",
    title: "#5a4418",
...
export function Awards({
  ...
  // Award Variant — laurier + titre. Couleurs métalliques injectées par level.
  if (variant === "award") {
    const c = badgePalette[level];
    const LaurelWreath = () => (
      <svg
        style={{ fill: c.fill }}
        className={cn(
    ...
    // Libellé FR de la pill (or / argent / bronze / platine)
    const levelLabels: Record<NonNullable<AwardsComponentProps["level"]>, string> = {
      gold: "Or",
      silver: "Argent",
      bronze: "Bronze",
      platinum: "Platine",
    ...
    return (
      ...
      >
        ...
        <div className="z-10 px-3 text-center">
          <div
            className={cn(
              "mt-2.5 mb-1 inline-block rounded-sm px-1.5 py-[1px] tracking-wider text-white text-[8px] font-semibold uppercase",
          ...
          {subtitle && (
            <h2
              ...
              className={cn("mb-1 text-[9px] md:text-[10px] font-light leading-snug")}
            >
              {subtitle}
            </h2>
          )}
          {recipient && (
            <p
              style={{ color: c.title }}
              className={cn("italic text-[9px] opacity-80")}
            >
              {recipient}
            </p>
          )}
          {date && (
            <div
              style={{ color: c.title }}
              className={cn("text-[9px] font-semibold mt-0.5")}
            >
              {date}
            </div>
          )}
  ...
  // Badge variant — colorisation par niveau (or / argent / bronze / platine).
  // Mise en page adaptée pour une rangée horizontale de 3 à 5 médailles :
  // titre compact (pas 4xl), kicker (subtitle) au-dessus du titre en petites
  // capitales, recipient (produit) en italique discret dessous.
  if (variant === "badge") {
    const c = badgePalette[level];
    const Badge = () => (
      <svg
        style={{ fill: c.fill }}
        className={cn("h-auto w-12 flex-none")}
        width="500.15"
        height="620.78"
    ...
    return (
      <div
        ...
        className={cn(
          "rounded-lg border-2 p-4 flex items-center gap-3 shadow-sm",
          className
        )}
      >
        <Badge />
        <div
          ...
          className={cn("border-l pl-3 min-w-0 flex-1")}
        >
          {subtitle && (
            <div
              style={{ color: c.subtitle }}
          ...
          <h3
            style={{ color: c.title }}
            className={cn("font-display text-lg leading-tight font-semibold tracking-tight")}
          >
            {title}
          </h3>
          {(recipient || date) && (
            <div
              style={{ color: c.subtitle }}
              className="mt-1 flex items-center gap-2 text-[11px] italic"
            >
              {recipient && <span>{recipient}</span>}
              {recipient && date && <span aria-hidden="true">•</span>}
  ...
  // Sticker Variant — conservé intact depuis 21st.dev (effets de stroke multiples)
  if (variant === "sticker") {
    return (
      <div
        className={cn(
      ...
      >
        <div className="relative text-center">
          <span
            ...
            style={{ transform: "translate(10px, 10px)", WebkitTextStroke: "10px white" }}
          >
            {title}
          </span>
          <span
            className="absolute block text-7xl font-black tracking-tighter dark:hidden"
            style={{ transform: "translate(10px, 10px)", WebkitTextStroke: "10px black" }}
          >
            {title}
          </span>
          <span
  ...
  // ID-card variant — conservé intact depuis 21st.dev
  if (variant === "id-card") {
    return (
      <div
        className={cn(
